...
import com.portingdeadmods.spaceploitation.networking.UpgradeWidgetOpenClosePayload;
import com.portingdeadmods.spaceploitation.networking.UpgradeWidgetSetSlotPositionsPayload;
...
public class UpgradePanelWidget extends PanelWidget {
    public static final ResourceLocation WIDGET_SPRITE = Spaceploitation.rl("widgets/widget_upgrade_right");
    public static final ResourceLocation WIDGET_OPEN_SPRITE = Spaceploitation.rl("widgets/upgrade_widget_open");
    ...
    public UpgradePanelWidget(int x, int y, boolean compressor) {
    ...
    public void setContext(MenuWidgetContext context) {
        ...
        PacketDistributor.sendToServer(new UpgradeWidgetSetSlotPositionsPayload(startY));
        if (menu instanceof PlanetSimulatorMenu psMenu) {
            psMenu.setUpgradeSlotPositions(startY);
        } else if (menu instanceof CompressorMenu cMenu) {
            cMenu.setUpgradeSlotPositions(startY);
        }
    }
...
//            guiGraphics.drawString(font, Component.translatable("redstone_signal_type." + IndustrialReforged.MODID + "." + signalType.getSerializedName()).withStyle(ChatFormatting.WHITE), getX() + 5, getY() + 54 + font.lineHeight + 2, -1);
        ...
        if (isHoveringSprite && context != null){
            try {
                ...
                if (!upgradeCounts.isEmpty()) {
                    ...
                    tooltip.add(Component.translatable("tooltip.spaceploitation.no_upgrades").withStyle(ChatFormatting.GRAY));
                }
                ...
                Spaceploitation.LOGGER.error("Error rendering upgrade tooltip", e);
    ...
    public void onWidgetResized(PanelWidget resizedWidget) {
        ...
        if (resizedWidget.isOpen()) {
            PacketDistributor.sendToServer(new UpgradeWidgetSetSlotPositionsPayload(startY + resizedWidget.getOpenHeight()));
            if (menu instanceof PlanetSimulatorMenu psMenu) {
                psMenu.setUpgradeSlotPositions(startY + resizedWidget.getOpenHeight());
            } else if (menu instanceof CompressorMenu cMenu) {
                cMenu.setUpgradeSlotPositions(startY + resizedWidget.getOpenHeight());
            }
        } else {
            PacketDistributor.sendToServer(new UpgradeWidgetSetSlotPositionsPayload(startY));
            if (menu instanceof PlanetSimulatorMenu psMenu) {
